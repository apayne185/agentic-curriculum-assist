/**
 * Picks the message an API route should send back to the client for an
 * error thrown by the LLM layer. Errors with a `userMessage` (see
 * lib/llm/types.ts) are written to be shown as-is to whoever is using the
 * app; everything else falls back to a generic message, since a raw
 * `err.message` can contain environment variable names, file paths, or
